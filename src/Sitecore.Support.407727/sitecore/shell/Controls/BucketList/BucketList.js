    ...
    self.SuccessHandler = function (multilist) {
        return function (request) {
            ...
            for (var i = 0; i < response.items.length; i++) {
                // Sitecore.Support.407727
                multilist.options[multilist.options.length] = new Option((response.items[i].DisplayName || response.items[i].Name) + ' (' + response.items[i].TemplateName + ' - ' + response.items[i].Bucket + ')', response.items[i].ItemId);
            }
